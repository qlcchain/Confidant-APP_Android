 ...
 * 带@功能的文本
 */
public class ATEmailEditText extends AppCompatEditText {

    private TInputConnection inputConnection;

    public ATEmailEditText(Context context) {
        super(context);
        init();
    }

    public ATEmailEditText(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public ATEmailEditText(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        for (DataSpan span : spans) {
            if (span != null) {
                ...
                if (selectionEnd == spanEnd) {
                    ...
                    int selectionEnd2 = text.length();
                    int selectionStart2 = 0;
                    DataSpan[] spans2 =  text.getSpans(selectionStart2, selectionEnd2, DataSpan.class);
                    int count = spans2.length;
                    return false;
                }
            }
        }
